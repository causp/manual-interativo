...
$(document).ready(function() {
    ...
    $('#entrada_carga_horaria_clt_ctps_digital').mask('00:00');
});

const salariosMinimos_clt_ctps_digital = [
...
let tipoCalculo_clt_ctps_digital = 'diario_clt_ctps_digital'; // Tipo de cálculo padrão (diário)

function atualizarSalarioMinimoCltCtpsDigital() {
    let dataContrato = document.getElementById('dataInicio_clt_ctps_digital').value;
    let avisoSalario = document.getElementById('avisoSalario_clt_ctps_digital');
    avisoSalario.innerHTML = ""; // Limpa mensagem anterior

    if (!dataContrato) return;

    let salario = salariosMinimos_clt_ctps_digital.find(salario => dataContrato >= salario.data);
    
    if (!salario) {
        avisoSalario.innerHTML = "Nenhum salário mínimo encontrado para a data selecionada.";
        document.getElementById('entrada_salario_minimo_clt_ctps_digital').value = "Data inválida para cálculo.";
    } else {
        document.getElementById('entrada_salario_minimo_clt_ctps_digital').value = `R$ ${salario.valor.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`;
    }
...
function atualizarTipoCalculoCltCtpsDigital() {
    tipoCalculo_clt_ctps_digital = document.getElementById('tipo_calculo_clt_ctps_digital').value;
    
    // Atualiza o label conforme o tipo de cálculo selecionado
    let labelCargaHoraria = document.getElementById('labelCargaHoraria_clt_ctps_digital');
    if (tipoCalculo_clt_ctps_digital === 'diario_clt_ctps_digital') {
...
function limparCamposCltCtpsDigital() {
    document.getElementById('dataInicio_clt_ctps_digital').value = '';
    document.getElementById('entrada_salario_minimo_clt_ctps_digital').value = '';
    document.getElementById('entrada_carga_horaria_clt_ctps_digital').value = '';
    document.getElementById('resultado_clt_ctps_digital').innerHTML = '';
    document.getElementById('erroCargaHoraria_clt_ctps_digital').innerHTML = '';
    document.getElementById('avisoSalario_clt_ctps_digital').innerHTML = '';
    document.getElementById('resultado_clt_ctps_digital').style.display = 'none';  // Esconde o resultado
}

function calcularPisoCltCtpsDigital() {
    var salarioMinimo = obterSalarioMinimoCltCtpsDigital(); // Função que pega o salário com base na data escolhida
    var cargaHorariaEmHoras = document.getElementById('entrada_carga_horaria_clt_ctps_digital').value;
    ...
    var resultadoDiv = document.getElementById('resultado_clt_ctps_digital');
    ...
    } else if (tipoCalculo_clt_ctps_digital === 'semanal_clt_ctps_digital') {
...
function obterSalarioMinimoCltCtpsDigital() {
    let dataContrato = document.getElementById('dataInicio_clt_ctps_digital').value;
    let salario = salariosMinimos_clt_ctps_digital.find(salario => dataContrato >= salario.data);
