...
$(document).ready(function() {
    ...
    $('#entrada_carga_horaria_prestador').mask('00:00');
});

const salariosMinimos_prestador = [
...
let tipoCalculo = 'diario_prestador'; // Tipo de cálculo padrão (diário)

function atualizarSalarioMinimoPrestador() {
    let dataContrato = document.getElementById('dataInicio_prestador').value;
    let avisoSalario = document.getElementById('avisoSalario_prestador');
    avisoSalario.innerHTML = ""; // Limpa mensagem anterior

    if (!dataContrato) return;

    let salario = salariosMinimos_prestador.find(salario => dataContrato >= salario.data);
    
    if (!salario) {
        avisoSalario.innerHTML = "Nenhum salário mínimo encontrado para a data selecionada.";
        document.getElementById('entrada_salario_minimo_prestador').value = "Data inválida para cálculo.";
    } else {
        document.getElementById('entrada_salario_minimo_prestador').value = `R$ ${salario.valor.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`;
    }
...
function atualizarTipoCalculoPrestador() {
    tipoCalculo = document.getElementById('tipo_calculo_prestador').value;
    
    // Atualiza o label conforme o tipo de cálculo selecionado
    let labelCargaHoraria = document.getElementById('labelCargaHoraria_prestador');
    if (tipoCalculo === 'diario_prestador') {
...
function limparCamposPrestador() {
    document.getElementById('dataInicio_prestador').value = '';
    document.getElementById('entrada_salario_minimo_prestador').value = '';
    document.getElementById('entrada_carga_horaria_prestador').value = '';
    document.getElementById('resultado_prestador').innerHTML = '';
    document.getElementById('erroCargaHoraria_prestador').innerHTML = '';
    document.getElementById('avisoSalario_prestador').innerHTML = '';
    document.getElementById('resultado_prestador').style.display = 'none';  // Esconde o resultado
}

function calcularPisoPrestador() {
    var salarioMinimo = obterSalarioMinimoPrestador(); // Função que pega o salário com base na data escolhida
    var cargaHorariaEmHoras = document.getElementById('entrada_carga_horaria_prestador').value;
    ...
    var resultadoDiv = document.getElementById('resultado_prestador');
    ...
    } else if (tipoCalculo === 'semanal_prestador') {
...
function obterSalarioMinimoPrestador() {
    let dataContrato = document.getElementById('dataInicio_prestador').value;
    let salario = salariosMinimos_prestador.find(salario => dataContrato >= salario.data);
